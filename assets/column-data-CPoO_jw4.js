const H=[{id:"next-generation-semiconductors-history-present-future",themes:["次世代半導体"],keywords:["次世代半導体","過去","現在","未来","関連銘柄"],category:"産業・技術",icon:"🧪",date:"2026/07/22",title:"次世代半導体の過去・現在・未来：市場構造と関連銘柄を徹底解説",summary:"次世代半導体の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。",body:`
H2: 次世代半導体とは何か
...
- キオクシアHD（285A）：強みはNANDフラッシュの量産基盤と先端メモリ開発力。弱み・確認点はメモリ市況の振幅が大きく、設備投資と価格下落の影響を受けやすい。
- アドバンテスト（6857）：強みはAI・高性能半導体向けテスターで高い競争力。弱み・確認点は主要顧客の設備投資と先端半導体サイクルへの依存が大きい。
- 東京エレクトロン（8035）：強みは前工程の幅広い装置群と顧客基盤、保守収益。弱み・確認点は半導体設備投資の減速や輸出規制の影響を受けやすい。
- 日立製作所（6501）：強みは電力・デジタル・産業設備を統合できる総合力。弱み・確認点は関連テーマの全社利益寄与が見えにくく大型案件の採算管理が必要。
- 信越化学工業（4063）：強みはシリコンウエハーと高純度材料の品質・量産力。弱み・確認点は大型投資の回収期間が長く半導体市況悪化時に稼働率が低下する。
- ルネサス（6723）：強みは車載・産業向けマイコンとアナログ半導体の顧客基盤。弱み・確認点は自動車・産業景気と在庫調整の影響を受けやすい。
- ディスコ（6146）：強みは切断・研削装置と消耗品で高い収益性。弱み・確認点は先端投資の変動と高い市場期待による株価変動が大きい。
- NEC（6701）：強みは量子・AI・通信・防衛を結ぶ研究開発と官公庁基盤。弱み・確認点は研究成果が商用利益へ転換するまで時間がかかる。
- 富士通（6702）：強みは計算基盤、量子ソフト、企業顧客への導入力。弱み・確認点はハード依存からサービス収益への転換速度が課題。
- SUMCO（3436）：強みは300ミリウエハーの世界的供給力。弱み・確認点は市況悪化時の価格・稼働率低下と巨額設備投資負担。
- レーザーテック（6920）：強みはEUVマスク検査という希少性の高い技術。弱み・確認点は顧客集中と製品サイクル、高い評価倍率への感応度。
- ソシオネクスト（6526）：強みは先端SoCの設計力と顧客別カスタム対応。弱み・確認点は案件の大型化で売上変動が大きく製造を外部へ依存。
...
- 三菱重工業（7011）：強みは原子力・タービン・大型機器を一体で扱う実績。弱み・確認点は大型案件の工期・原価超過と政策依存。
- 日立製作所（6501）：強みは電力・デジタル・産業設備を統合できる総合力。弱み・確認点は関連テーマの全社利益寄与が見えにくく大型案件の採算管理が必要。
- IHI（7013）：強みは極低温、宇宙、防衛、エネルギー機器の複合技術。弱み・確認点は大型プロジェクトの採算変動と個別案件リスク。
- NEC（6701）：強みは量子・AI・通信・防衛を結ぶ研究開発と官公庁基盤。弱み・確認点は研究成果が商用利益へ転換するまで時間がかかる。
- 三菱電機（6503）：強みは電力制御、FA、センサー、宇宙機器の広い技術基盤。弱み・確認点は事業範囲が広くテーマ寄与が薄まりやすい。
- 富士通（6702）：強みは計算基盤、量子ソフト、企業顧客への導入力。弱み・確認点はハード依存からサービス収益への転換速度が課題。
- 住友重機械工業（6302）：強みは精密機器、加速器、極低温関連の専門技術。弱み・確認点は受注産業の設備投資サイクルと案件採算の影響。
- 日本電子（6951）：強みは電子顕微鏡・分析装置で研究開発需要を取り込める。弱み・確認点は研究予算と先端設備投資に左右されやすい。
...
- 日立製作所（6501）：強みは電力・デジタル・産業設備を統合できる総合力。弱み・確認点は関連テーマの全社利益寄与が見えにくく大型案件の採算管理が必要。
- IHI（7013）：強みは極低温、宇宙、防衛、エネルギー機器の複合技術。弱み・確認点は大型プロジェクトの採算変動と個別案件リスク。
- 東京電力HD（9501）：強みは首都圏の大規模顧客基盤と原子力資産。弱み・確認点は事故対応費用、規制、再稼働不確実性、財務負担。
- 関西電力（9503）：強みは原子力比率が高く再稼働の収益効果が大きい。弱み・確認点は設備停止や規制変更時の利益変動が大きい。
- 中部電力（9502）：強みは発電・送配電・小売の規模と安定顧客基盤。弱み・確認点は燃料価格と原子力再稼働の不確実性。
- GSユアサ（6674）：強みは産業用蓄電池と非常用電源の実績。弱み・確認点は原材料価格と大型案件の採算変動。
- 住友重機械工業（6302）：強みは精密機器、加速器、極低温関連の専門技術。弱み・確認点は受注産業の設備投資サイクルと案件採算の影響。
- 北海道電力（9509）：強みは泊原発再稼働時の燃料費改善余地。弱み・確認点は地域需要の制約と再稼働時期の不確実性。
- 九州電力（9508）：強みは原子力稼働による低燃料費電源の比率。弱み・確認点は電力価格制度と設備トラブルの影響。
- 東北電力（9506）：強みは東北地域の電力基盤と再エネ接続需要。弱み・確認点は燃料価格、災害、設備投資負担。
...
- 三菱電機（6503）：強みは電力制御、FA、センサー、宇宙機器の広い技術基盤。弱み・確認点は事業範囲が広くテーマ寄与が薄まりやすい。
- 富士通（6702）：強みは計算基盤、量子ソフト、企業顧客への導入力。弱み・確認点はハード依存からサービス収益への転換速度が課題。
- NTT（9432）：強みは通信基盤、研究所、量子通信・光技術の長期研究。弱み・確認点は商用化の時間軸が長く規制・料金政策の影響。
- 住友商事（8053）：強みは海外ネットワークと新技術への投資・事業化力。弱み・確認点は投資先評価損と技術選定リスク。
- デンソー（6902）：強みは車載半導体・制御技術と量産品質。弱み・確認点は自動車生産、顧客集中、先行投資負担。
...
- ソニーグループ（6758）：強みはセンサー、音響、映像、コンテンツを統合できる。弱み・確認点はウェアラブル事業の全社寄与が小さく製品競争が激しい。
- 村田製作所（6981）：強みは小型高性能部品と無線モジュールの量産力。弱み・確認点はスマホ需要と顧客在庫調整の影響。
- TDK（6762）：強みはセンサー、電池、受動部品を横断した製品群。弱み・確認点は電子機器需要と原材料・為替の変動。
- ローム（6963）：強みは低消費電力・センサー・電源半導体の技術。弱み・確認点は設備投資負担と半導体市況の影響。
- 京セラ（6971）：強みはセラミック部品と通信・光学の幅広い基盤。弱み・確認点は事業分散により成長テーマの利益寄与が見えにくい。
- カシオ計算機（6952）：強みは腕時計ブランドと耐久設計、販売網。弱み・確認点はスマートウォッチ市場でOS・アプリ基盤が弱い。
- ニコン（7731）：強みは光学・精密加工とAR関連の技術資産。弱み・確認点は消費者向け製品縮小と新規事業の収益化。
- アルプスアルパイン（6770）：強みは小型入力部品、センサー、通信モジュール。弱み・確認点は自動車・スマホ需要と価格競争。
- シチズン時計（7762）：強みは時計の省電力技術とブランド、精密加工。弱み・確認点はスマート端末との競争と成熟市場依存。
- セイコーグループ（8050）：強みは時計ブランド、精密部品、電子デバイス。弱み・確認点は事業規模が小さく消費動向・為替の影響。
...
- 日本ペイントHD（4612）：強みはアジアを中心とする規模と建築塗料基盤。弱み・確認点は買収依存、地域景気、原材料価格の影響。
- 関西ペイント（4613）：強みは自動車・工業塗料と海外顧客基盤。弱み・確認点は自動車生産と原材料価格、地域別採算。
- 中国塗料（4617）：強みは船舶用塗料の専門性と世界的顧客基盤。弱み・確認点は造船市況と海運投資への依存。
- DIC（4631）：強みは顔料、樹脂、包装・電子材料の総合力。弱み・確認点は事業構造が複雑で原料価格と低採算事業の影響。
- 東洋インキSCHD（4634）：強みは印刷インキから機能性材料へ展開。弱み・確認点は印刷市場縮小と成長分野への転換速度。
- 日本特殊塗料（4619）：強みは自動車防音材と特殊機能塗料。弱み・確認点は自動車生産と特定顧客への依存。
- 神東塗料（4616）：強みは工業・建築向けの専門製品。弱み・確認点は規模が小さく原料高と価格競争の影響。
...
- NEC（6701）：強みは量子・AI・通信・防衛を結ぶ研究開発と官公庁基盤。弱み・確認点は研究成果が商用利益へ転換するまで時間がかかる。
- 三菱電機（6503）：強みは電力制御、FA、センサー、宇宙機器の広い技術基盤。弱み・確認点は事業範囲が広くテーマ寄与が薄まりやすい。
- セコム（9735）：強みは機械警備の契約基盤とブランド、継続収入。弱み・確認点は人件費上昇と成熟国内市場。
- 綜合警備保障（ALSOK）（2331）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- キヤノン電子（7739）：強みは精密機器、宇宙、画像処理の技術。弱み・確認点は警備テーマとの直接的な売上寄与が限定的。
- 東洋テック（9686）：強みは関西圏の地域基盤と法人顧客。弱み・確認点は地域集中、規模、人件費上昇。
...
- フジクラ（5803）：強みは光ファイバーと高密度配線でAIデータセンター需要を取り込む。弱み・確認点は需要急増後の反動と顧客・製品集中。
- 古河電気工業（5801）：強みは光通信、電力ケーブル、自動車部品の幅広さ。弱み・確認点は低採算事業と構造改革、原料価格の影響。
- 住友電気工業（5802）：強みは電線、光通信、ワイヤーハーネスの総合力。弱み・確認点は自動車生産と銅価格、事業規模ゆえの利益率。
- SWCC（5805）：強みは電力ケーブルと送配電更新需要への集中。弱み・確認点は原材料価格、案件集中、規模の制約。
- タツタ電線（5809）：強みは機能性電線・電子材料の専門性。弱み・確認点は事業規模と顧客集中、上場・資本政策の変化。
- 三菱マテリアル（5711）：強みは銅製錬、加工、リサイクルの一貫基盤。弱み・確認点は資源価格、製錬マージン、事業再編リスク。
- 住友金属鉱山（5713）：強みは鉱山権益、製錬、電池材料の統合力。弱み・確認点は金属価格、資源国、鉱山投資の変動。
- JX金属（5016）：強みは銅・半導体材料の高機能製品。弱み・確認点は銅価格と先端材料投資の景気循環。
...
- イビデン（4062）：強みはAI向け高性能パッケージ基板と主要顧客実績。弱み・確認点は顧客集中、巨額投資、歩留まりの影響。
- メイコー（6787）：強みは車載・高多層基板の量産力。弱み・確認点は設備投資負担と自動車・中国需要。
- 日本シイエムケイ（6958）：強みは車載プリント基板の顧客基盤。弱み・確認点は収益性の低さと価格競争。
- 太陽ホールディングス（4626）：強みはソルダーレジストで高シェアと高収益。弱み・確認点は半導体・基板需要の変動と新規事業投資。
- TOWA（6315）：強みは半導体封止装置で先端パッケージ需要を取り込む。弱み・確認点は装置受注の変動、顧客投資サイクル。
- レゾナックHD（4004）：強みは封止材・基板材料の幅広いポートフォリオ。弱み・確認点は大型買収後の財務負担と事業再編。
- 日本マイクロニクス（6871）：強みはプローブカードでメモリ・先端半導体検査に強い。弱み・確認点はメモリ投資サイクルと顧客集中。
- ディスコ（6146）：強みは切断・研削装置と消耗品で高い収益性。弱み・確認点は先端投資の変動と高い市場期待による株価変動が大きい。
- 東京応化工業（4186）：強みは高純度フォトレジスト・電子材料の技術。弱み・確認点は顧客認定の長期化と半導体市況。
...
- ニデック（6594）：強みはモーター、ファン、冷却部品の量産力。弱み・確認点は買収統合、EV投資、利益率改善の不確実性。
- 富士電機（6504）：強みはパワー半導体、受配電、UPSの総合力。弱み・確認点は設備投資と大型案件、原材料の影響。
- 明電舎（6508）：強みは受変電・電力品質設備の実績。弱み・確認点は国内設備投資と案件採算への依存。
- 高砂熱学工業（1969）：強みは大規模空調・データセンター施工と保守。弱み・確認点は建設人員不足、資材高、工期管理。
- 新晃工業（6458）：強みは大型空調機器と更新需要。弱み・確認点は建設市況、部材価格、顧客案件の偏り。
- 日東工業（6651）：強みは配電盤・キャビネットの製品基盤。弱み・確認点は建設・設備投資と材料価格の影響。
- ダイヘン（6622）：強みは電力変換、変圧器、溶接・ロボット技術。弱み・確認点は設備投資サイクルと原材料価格。
- 三社電機製作所（6882）：強みは電源装置とパワー半導体の専門性。弱み・確認点は規模、顧客集中、需要変動。
- サンケン電気（6707）：強みは電源半導体と高効率電力変換。弱み・確認点は事業再編、財務、半導体市況。
...
- 日本M&Aセンター（2127）：強みは全国の金融・会計ネットワークと案件供給力。弱み・確認点は過去の不適切処理による信頼回復と人材生産性。
- M&A総研HD（9552）：強みはDXを活用した高いコンサルタント生産性。弱み・確認点は高成長期待、採用、案件品質への依存。
- ストライク（6196）：強みは公認会計士基盤と中堅案件の専門性。弱み・確認点は案件成約時期による業績変動。
- オンデック（7360）：強みは中小企業M&Aへの専門対応。弱み・確認点は規模が小さく人材・案件集中の影響。
- 名南M&A（7076）：強みは東海地域と士業ネットワーク。弱み・確認点は地域集中と上場市場での流動性。
- ブティックス（9272）：強みは介護・医療分野のM&Aと展示会顧客基盤。弱み・確認点は特定業界依存と人材採用・定着。
