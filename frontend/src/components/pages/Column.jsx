...
const CAT_COLORS = {
  ...
  '光ファイバー・光部品':     { bg:'rgba(74,158,255,0.1)',  color:'#4a9eff',  border:'rgba(74,158,255,0.25)' },
...
export default function Column({ initialArticleId = null, onNavigate }) {
  ...
  const THEME_CATS = [
    ...
    'LNG','石油','蓄電池','資源（水素・ヘリウム・水）','IOWN','光ファイバー・光部品',
      ...
      <div style={{ padding:'20px 32px 60px', maxWidth:'760px', margin:'0 auto' }}>
                ...
                <div key={theme} style={{
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
                    ...
                    {THEME_ARTICLE_MAP[theme] && THEME_ARTICLE_MAP[theme] !== col.id && (
                      <button
                        onClick={() => onNavigate('コラム・解説', THEME_ARTICLE_MAP[theme])}
                        style={{ padding:'5px 12px', borderRadius:'5px', fontSize:'11px', fontWeight:600,
        ...
        {(() => {
          const CAT_TO_THEME = {
            ...
            'IOWN':'IOWN','光ファイバー・光部品':'光ファイバー','通信':'通信',
