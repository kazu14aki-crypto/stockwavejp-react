...
const SECTIONS = [
  {
    icon: '📊',
    title: 'テーマ一覧',
    desc: '72テーマ以上の騰落率・出来高・売買代金を一覧比較できるページです。データはInfoway（日本株APIサービス）から取得しています。',
    items: [
      '上部の期間セレクター（1日/1週間/1ヶ月/3ヶ月/6ヶ月/1年）で表示期間を切り替えられます。※Free プランは3ヶ月・6ヶ月・1年のみ選択可。',
  ...
  },
  {
    ...
    items: [
      ...
      '銘柄表の銘柄名・注目銘柄ピックアップのカード・ヒートマップのバブルをクリック（スマホは2タップ目）すると、その銘柄の「銘柄詳細」ページに移動できます。',
      'PER・来期PER・PBR・来期PBR・PEG・来期PEGの6列はサブスクリプション加入者限定です。未加入の場合は🔒マークが表示されます。',
    ]
  },
  {
    icon: '📋',
    ...
    items: [
      ...
      '銘柄表の銘柄名・出来高/売買代金ランキングの銘柄名・バブルのクリックで「銘柄詳細」ページに移動できます。',
      'PER等のバリュエーション列はサブスクリプション加入者限定（未加入は🔒表示）です。',
    ]
  },
  {
    icon: '📈',
    title: '銘柄詳細',
    desc: '個別銘柄の株価・テクニカル・指標・大株主を1ページで確認できます。テーマ別詳細や市場別詳細の銘柄名クリックから移動します。',
    items: [
      '1年チャート（累積騰落率）と、25日線・75日線からの乖離率、1年高値からの位置、RSI(14)を表示します。',
      '所属テーマがチップで表示され、クリックでそのテーマの詳細ページへ移動できます。',
      'PER・PBR・PEG等のバリュエーションはサブスクリプション加入者限定です（データ提供元との連携準備中の項目は「—」表示になります）。',
      '大株主の状況（有価証券報告書ベース）を表示します。無料プランは上位3名まで、サブスク加入者は上位10名すべてと属性バッジ（創業家・信託口・外国機関など）・保有比率を確認できます。',
      '大株主のより詳しい分析（属性構成バー・前期比の増減）は機関投資家保有ページで確認できます。',
    ]
  },
  {
    icon: '🎨',
  ...
  },
  {
    icon: '🏦',
    title: '機関投資家保有・大株主',
    desc: '金融庁EDINETの開示データをもとに「誰がその株を持っているか」を2つの視点で確認できるページです。',
    items: [
      '【大株主を調べる】銘柄名または証券コードで検索すると、有価証券報告書の「大株主の状況」上位10名を表示します。サイト収録の全銘柄が検索対象です。',
      '各株主には属性バッジ（個人＝創業者一族の可能性・信託口＝年金/投信の名義・外国機関・事業法人・自己株式など）が付き、上位10名の属性構成バーと自動診断コメント（オーナー系企業/機関投資家主導など）が表示されます。',
      '前回の有価証券報告書と比較した保有比率の増減（▲▼）と新規登場（NEW）も確認できます。「有報データ未取得」と表示される銘柄は、週次の自動取得バッチの巡回後に表示されます。',
      '【大量保有報告書（5%ルール）】発行済株式の5%超を取得した投資家のリアルタイムな報告を確認できます。有報が年1回の定点観測なのに対し、こちらは「今まさに動いている大口資金」を捉える視点です。',
      '保有割合5〜7%の新規報告は注目サイン。10%超は経営参画・アクティビストの可能性があります。増加トレンドが続く場合は積み増し中のサインです。',
      '信託口（マスタートラスト等）は信託銀行自身の投資ではなく、年金基金や投資信託が名義上信託銀行として記載されているものです。読み方の詳細はコラム「大株主を読めば企業がわかる」をご覧ください。',
    ]
  },
  {
    icon: '📰',
    ...
    items: [
      ...
      '各レポートには「特に強かったテーマ・銘柄」「特に弱かったテーマ・銘柄」の個別解説と、週末の値動きの読み方、翌週の注目イベント・テーマ・銘柄を掲載しています。',
...
const QA = [
  ...
  {
    q: 'PER・PBR・PEGの列が🔒や「—」になっています',
    a: 'バリュエーション指標（PER・来期PER・PBR・来期PBR・PEG・来期PEG）はサブスクリプション加入者限定の機能です。🔒はプラン未加入、「—」はデータ提供元との連携準備中を意味します。銘柄詳細ページの大株主情報も、無料プランは上位3名まで・サブスク加入で全員表示となります。',
  },
  {
    q: '売買代金の単位は何ですか？',
  ...
  {
    q: 'データソースはどこですか？',
    a: 'データはInfoway（data.infoway.io）より取得しています。Infowayは国内株式の日次OHLCV・リアルタイムデータを提供する商用APIです。大量保有報告書データは金融庁EDINETより取得しています。Infowayからのデータ取得は行っていません。',
  },
]
