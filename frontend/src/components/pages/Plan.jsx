...
export default function Plan() {
  ...
  const s    = { padding:'24px 16px 60px', maxWidth:'860px', margin:'0 auto' }
  ...
  const PLANS = [
    ...
    },
    {
      ...
      monthly: { price:'¥1,180', label:'単月契約', perDay: perDay(1180) },
      yearly:  { price:'¥11,800', label:'年間契約', discount:'約17%OFF！', perDay: perDay(Math.round(11800/12)) },
          ...
          <div key={p.name} style={{
            ...
            <div style={{
              ...
              <div style={{ display:'flex', alignItems:'baseline', gap:'4px' }}>
                ...
                {p.monthly.label !== '永久無料' && (
                  <span style={{ fontSize:'12px', color:'var(--text3)' }}>/月</span>
                )}
              </div>
              {p.monthly.perDay && (
                <div style={{ fontSize:'11px', color:p.color, fontWeight:600, marginTop:'4px' }}>
                  一日あたり約{p.monthly.perDay}円！
                </div>
              )}
              {p.monthly.label === '永久無料' && (
                <div style={{ fontSize:'11px', color:'#4a9eff', fontWeight:600, marginTop:'4px' }}>
                  ずっと無料
                </div>
              )}
            </div>
            ...
            {p.yearly && (
              <div style={{
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:'4px' }}>
                  <span style={{ fontSize:'22px', fontWeight:800, color:p.color, fontFamily:'var(--mono)' }}>
                    {p.yearly.price}
                  </span>
                  <span style={{ fontSize:'11px', color:'var(--text3)' }}>/年</span>
                </div>
                <div style={{ fontSize:'11px', color:p.color, fontWeight:600, marginTop:'4px' }}>
                  一日あたり約{p.yearly.perDay}円！
                </div>
              </div>
            )}
