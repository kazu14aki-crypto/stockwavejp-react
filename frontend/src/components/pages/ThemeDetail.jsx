        ...
        {items.map((s,i)=>{
          ...
          return (
            <g key={s.ticker}>
              ...
              <text x={x+bW/2} y={H-PB+12} textAnchor="middle" fill="var(--text3)" fontSize="9" fontFamily="DM Sans">{s.name.length>4?s.name.slice(0,4)+'…':s.name}</text>
            </g>
          )
        })}
...
export default function ThemeDetail() {
  ...
  return (
    <div style={{ padding:'28px 32px 48px' }}>
      <h1 style={{ fontSize:'24px',fontWeight:700,letterSpacing:'-0.02em',color:'#e8f0ff',marginBottom:'4px' }}>テーマ別詳細</h1>
      <p style={{ fontSize:'12px',color:'var(--text3)',marginBottom:'20px' }}>テーマを選択して構成銘柄の詳細を確認</p>
