...
function KpiCard({ label, value, valueColor, sub, delay=0, loading=false }) {
  return (
    ...
    >
      ...
      <div style={{ fontFamily:'var(--mono)', fontSize:'24px', fontWeight:700, letterSpacing:'-0.02em', lineHeight:1, marginBottom:'6px', color:valueColor||'var(--text)' }}>
        {loading ? <Dots /> : value}
      </div>
  ...
  return (
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'8px', padding:'12px 14px',
      ...
      <div>
        ...
        <div style={{ fontFamily:'var(--mono)', fontSize:'15px', fontWeight:700, color }}>
...
function SHead({ title }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'10px', margin:'20px 0 12px' }}>
      <span style={{ fontSize:'12px', fontWeight:700, color:'var(--text2)', letterSpacing:'0.08em', textTransform:'uppercase', whiteSpace:'nowrap' }}>{title}</span>
  ...
  return (
    <div style={{ padding:'24px 32px 48px' }}>
      ...
      <div style={{
        ...
        padding:'24px 28px', marginBottom:'20px', animation:'fadeUp 0.5s ease both',
      }}>
        <h1 style={{ fontSize:'24px', fontWeight:700, letterSpacing:'-0.02em', color:'var(--text)', marginBottom:'8px' }}>
          <span style={{ color:'var(--logo-red)' }}>Stock</span>Wave<span style={{ color:'var(--logo-red)', fontSize:'14px' }}>JP</span>
        </h1>
        {/* PC:1行 / SP:折り返し */}
        ...
        {NEWS_LIST.map((n,i)=>{
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{ fontSize:'9px', fontWeight:700, padding:'1px 7px', borderRadius:'20px',
                background:tc.bg, color:tc.color, border:`1px solid ${tc.border}`, whiteSpace:'nowrap' }}>
                {n.tag}
              </span>
              <span style={{ fontSize:'12px', fontWeight:600, color:'var(--text)', flex:1 }}>{n.title}</span>
              <span style={{ fontSize:'10px', color:'var(--text3)', fontFamily:'var(--mono)', whiteSpace:'nowrap' }}>{n.date}</span>
            </div>
          )
        })}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'4px' }} className="top-kpi">
        ...
        <KpiCard delay={0.15} loading={loading} label="資金流入TOP"
          value={<span style={{ fontSize:'15px' }}>{s?.top?.theme||'-'}</span>}
        ...
        <KpiCard delay={0.2} loading={loading} label="資金流出TOP"
          value={<span style={{ fontSize:'15px' }}>{s?.bot?.theme||'-'}</span>}
      ...
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px' }} className="macro-grid">
