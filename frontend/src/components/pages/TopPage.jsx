...
export default function TopPage({ onNavigate }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      setLoading(true)
      try {
        const [themesRes, macroRes] = await Promise.all([
  ...
  return (
    <div style={{ padding: '28px 32px 48px' }}>
      ...
      <div style={{
        ...
        <p style={{ fontSize: '14px', color: 'var(--text2)', lineHeight: 1.7, maxWidth: '600px' }}>
          日本株テーマ別の騰落率・出来高・売買代金をリアルタイムで追跡。
          どのテーマに資金が集まっているかを視覚的に把握できます。
        </p>
        <div style={{ display: 'flex', gap: '10px', marginTop: '20px', flexWrap: 'wrap' }}>
          <button onClick={() => onNavigate('テーマ一覧')} style={btnStyle}>
            📊 テーマ一覧を見る →
          </button>
          <button onClick={() => onNavigate('市場別ランキング')} style={{ ...btnStyle, background: 'transparent', color: 'var(--text2)', border: '1px solid var(--border)' }}>
            📋 市場別ランキング
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }} className="top-grid">
        {/* マクロ指標 */}
        <div>
            ...
            {NEWS_LIST.map((n, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)', marginBottom: '4px' }}>{n.title}</div>
