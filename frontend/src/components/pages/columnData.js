...
const COLUMNS = [
  {
    id: 'domestic-demand-defensive-theme-2026',
    themes: ['内需・ディフェンシブ'],
    keywords: ['内需', 'ディフェンシブ', '個人消費', '価格転嫁', '生活必需品', '実質所得'],
    category: 'テーマ解説',
    icon: '🏠',
    date: '2026/08/09',
    title: '内需・ディフェンシブテーマを読み解く：景気に強い、だけでは足りない理由',
    summary: '国内需要を軸にし、景気変動の影響を比較的受けにくいとされる企業群を、需要の安定性、価格転嫁、賃金・物価、金利の観点から整理します。',
    body: `
H2: 内需・ディフェンシブとは何か
...
`,
  ...
  {
    id: 'weak-yen-benefit-theme-2026',
    themes: ['円安メリット'],
    keywords: ['円安メリット', '為替', '輸出', '海外売上', '為替ヘッジ', '輸入コスト'],
    category: 'テーマ解説',
    icon: '💱',
    date: '2026/08/09',
    title: '円安メリットテーマを読み解く：輸出企業だけを買えばよい、ではない',
    summary: '円安は海外売上の円換算額を押し上げ得ますが、輸入原材料、海外生産、ヘッジ、価格競争力も同時に動きます。企業ごとの本当の為替感応度を確認する方法を解説します。',
    body: `
H2: 円安メリットは、為替レートだけで決まらない
...
`,
  ...
  {
    id: 'electric-utilities-theme-2026',
    themes: ['電力会社'],
    keywords: ['電力需要', '脱炭素電源', '送配電網', '容量市場', '電力料金', 'エネルギー基本計画'],
    category: 'テーマ解説',
    icon: '⚡',
    date: '2026/08/09',
    title: '電力会社テーマを読み解く：AI時代の需要増、脱炭素投資、料金制度をどう見るか',
    summary: '電力会社は「安定配当」のイメージだけでは捉えきれない局面に入っています。需要の変化、発電構成、送配電網への投資、燃料価格と制度の関係を、投資判断の前提として整理します。',
    body: `
H2: 電力会社は「電気を売る会社」から、電力システムを支える会社へ
...
`,
  ...
  {
    id: 'hydrogen-helium-water-resources-theme-2026',
    themes: ['資源（水素・ヘリウム・水）'],
    keywords: ['水素', 'ヘリウム', '水資源', 'サプライチェーン', '脱炭素', '資源安全保障'],
    category: 'テーマ解説',
    icon: '💧',
    date: '2026/08/09',
    title: '資源（水素・ヘリウム・水）テーマを読み解く：希少性より「使える供給網」を見る',
    summary: '水素、ヘリウム、水は性質も市場も異なりますが、共通するのは供給網、品質、設備、規制が価値を決める点です。話題性に流されず、社会実装の条件を整理します。',
    body: `
H2: 一つの資源テーマに見えて、投資論点は三つに分かれる

「資源（水素・ヘリウム・水）」は、同じ棚に置かれがちですが、価格形成も用途も異なります。水素は脱炭素化に向けたエネルギー媒体・原料としての社会実装、ヘリウムは半導体・医療・研究などで求められる高純度ガスの安定確保、水は生活・産業・農業を支える基盤資源としての処理・再利用・漏水対策が中心です。

共通して重要なのは「希少そうだから伸びる」という見方ではありません。必要な品質で、必要な場所に、継続して届けられるか。製造、精製、輸送、貯蔵、利用設備、保安、長期契約までをつないだ供給網が成立するかが、事業性を左右します。

H2: 水素は製造技術より、需要とインフラの同時立ち上げが鍵

水素は利用時にCO2を排出しない一方、環境価値は製造時のエネルギー源で変わります。また、気体のままではかさばるため、液化、圧縮、アンモニアなどの媒体化を含む輸送・貯蔵コストが課題です。発電、製鉄、化学、モビリティなどの需要をまとめ、供給側と利用側が長期に量を約束できるかが重要になります。

日本では2023年改定の水素基本戦略と、水素社会推進法を踏まえ、低炭素水素等の大規模サプライチェーンを支援する枠組みが整えられてきました。投資家が見るべきなのは、実証という言葉の多さではなく、供給量、稼働開始時期、顧客との引取契約、コスト差の補填、保安体制まで具体化しているかです。

H2: ヘリウムは「代替しにくさ」と調達集中リスク

ヘリウムは、低温環境、半導体製造、光ファイバー、医療機器、研究用途などで使われます。用途によっては代替が容易ではなく、純度・安定供給・回収設備が重要です。一方で、供給地域や大型設備への依存度が高い場合、地政学、設備トラブル、物流の混乱が価格や入手性に影響します。

関連企業をみる際は、単純なガス販売量だけでなく、調達先の分散、長期契約、充填・配送網、使用後の回収・再利用、顧客が必要とする純度への対応を確認します。供給不安がある局面では在庫評価の上振れも起こり得ますが、恒常的な競争力と短期的な需給ひっ迫は分けて考える必要があります。

H2: 水は量だけでなく、処理・再利用・データ管理の産業

水不足や豪雨、老朽化した管路、工場の超純水需要などは、水関連投資の入口になります。重要なのは水を「売る」ことだけではなく、浄水、下水処理、漏水検知、膜・ろ過、再利用、運転管理までを含むことです。地域ごとの水質・地形・制度に合わせた個別性が高く、導入後の保守サービスが収益の継続性につながる場合もあります。

H2: テーマを見るためのチェックリスト

1. 何を、誰が、どの用途で買うのか
2. 製造・精製から輸送・貯蔵・保安までのボトルネックはどこか
3. 補助金がなくても採算に近づく道筋があるか
4. 長期契約、保守、回収・再利用など継続収益があるか
5. 地政学・天候・規制・安全性のリスクを分散できているか

H2: まとめ：資源の話は「量」より「供給の質」を確認する

水素・ヘリウム・水はいずれも、社会に必要だからといって自動的に高収益になるわけではありません。大きな初期投資、契約獲得、安全性、地域調整が必要です。その一方、供給網を設計し、品質と安定性で顧客の操業を支えられる企業には強みが生まれます。ニュースは技術発表だけでなく、商用化の数量、相手先、開始時期、投資回収の前提まで確認する習慣を持ちましょう。

H2: 水素を「色」だけで判断しない

水素は製造方法に応じてグリーン、ブルー、グレーなどと呼ばれることがあります。しかし、呼び名だけで環境性や採算性が決まるわけではありません。電力や原料の調達方法、製造設備の稼働率、CO2の回収・貯留の実効性、輸送時のエネルギー、認証ルールまでを含めて見なければなりません。同じ「低炭素水素」でも、利用場所、契約期間、補助制度、輸送距離でコスト構造は大きく変わります。

企業発表では、製造能力の数字が先に出やすい一方で、実際に大切なのは販売量です。供給プロジェクトであれば引取先が確保されているか、利用側であれば燃料転換に必要な設備と運用ノウハウがあるか、双方で価格差をどう埋めるかを確認します。水素社会は設備単体の市場ではなく、供給・需要・保安・資金を同時に成立させる市場です。実証の成功と商用の継続採算は別物であることを意識しましょう。

H2: ヘリウムと水は「止められない顧客」を支えられるか

ヘリウムでは、顧客側の工程を止めないことが最大の価値になり得ます。半導体、医療、研究などは、供給の中断や品質のばらつきが生産・検査に影響する場合があります。そのため、単価の安さだけでなく、調達先の分散、容器の回収、緊急時の配送、純度管理、代替ガスや節約技術の提案までが競争力になります。需給が逼迫した局面の価格上昇を、永続的な高収益と見なさない慎重さも必要です。

水関連では、人口減少地域と工業集積地域で課題が異なります。前者では老朽管路の維持管理や人手不足、後者では超純水・排水再利用・安定供給が重要になります。水処理装置の初期受注だけでなく、薬品、膜の交換、遠隔監視、保守契約がどれだけ継続収益になるかを確認します。災害や渇水が注目を集めた時ほど、短期の受注期待と、自治体・企業の予算執行を伴う長い案件サイクルを分けて考える必要があります。

H2: プロジェクト投資のリスクを分解する

資源テーマには、建設遅延、許認可、環境影響、調達国の政治リスク、為替、金利、顧客の稼働計画といった複数のリスクが重なります。大規模案件では、一つの遅延が他の設備の稼働開始も遅らせ、資金負担を増やすことがあります。投資家は契約額や補助金額だけではなく、自己資本・借入・共同出資の比率、契約解除条項、開始時期、顧客への転嫁可能性を確認しましょう。大きなテーマほど、派手な技術ニュースより着実な契約・稼働・保守実績の積み上げを評価する姿勢が有効です。

H2: 国際競争と資源安全保障の視点

水素やヘリウムの供給網は、単に国内企業の技術競争ではなく、調達国、港湾、船舶、貯蔵基地、国際規格を含む国際競争です。特定地域・特定設備への依存が高いと、紛争、制裁、自然災害、設備事故、航路の混乱によって供給が止まるリスクがあります。調達先を増やすことは安心材料になりますが、すべてを分散すればコストも上がり得ます。安定供給と経済性のどこで折り合いをつけるかが経営課題です。

水資源でも、装置・膜・計測器・薬品・運転データの供給が一つの国や企業に偏ると、長期保守に影響します。利用者にとっては、初期導入費が低いことより、数十年にわたり部品と技術者を確保できることが大切な場合があります。企業の海外展開では、受注額だけでなく、現地パートナー、保守拠点、代金回収、政治・為替リスクをどう管理するかを確認しましょう。

H2: 開示資料から商用化の確度を読む

実証、基本合意、検討開始、最終投資決定、建設開始、試運転、商業運転開始は意味が異なります。ニュースの見出しだけでは進捗を過大評価しやすいため、案件がどの段階にあるかを表にして追うのが有効です。最終投資決定の前なら、資金、許認可、顧客契約が条件として残っていることがあります。商業運転開始後も、設計どおりの稼働率と販売量が出ているかを確認する必要があります。

また、売上計上が機器販売なのか、ガス・水の継続供給なのか、保守サービスなのかで、収益の再現性は変わります。大型案件の受注残が大きくても、粗利率や運転資金が悪化すれば株主価値に結び付きません。資源テーマでは技術的な夢と事業上の現実の距離を測ることが、最も重要な分析になります。

主な参考資料：経済産業省・資源エネルギー庁「水素基本戦略」「水素社会実現に向けた取組」、エネルギー白書2025
`,
  },
  {
    id: 'saas-theme-2026',
    themes: ['SaaS'],
    keywords: ['SaaS', 'DX', 'ARR', '解約率', '顧客獲得コスト', '生成AI'],
    category: 'テーマ解説',
    icon: '☁️',
    date: '2026/08/09',
    title: 'SaaSテーマを読み解く：売上成長だけでは足りない、継続率と単価をどう見るか',
    summary: 'SaaSは継続課金モデルの強みがある一方、顧客獲得コストと解約率が収益性を左右します。DX・生成AIの追い風を、実際のKPIに落として読むための基礎を解説します。',
    body: `
H2: SaaSとは、ソフトを売り切るのではなく使い続けてもらう事業
...
`,
  ...
  {
    id: 'building-materials-theme-2026',
    themes: ['建築資材'],
    keywords: ['建築資材', 'セメント', '鋼材', '木材', '住宅着工', '価格転嫁'],
    category: 'テーマ解説',
    icon: '🏗️',
    date: '2026/08/09',
    title: '建築資材テーマを読み解く：着工件数、価格転嫁、更新需要を一緒に見る',
    summary: '建築資材は住宅・非住宅・公共工事の需要に連動しますが、原材料・物流・エネルギーコストの影響も大きい分野です。数量と単価、更新投資の三層で読み解きます。',
    body: `
H2: 建築資材は「建設需要の代理指標」ではない
...
`,
  ...
  {
    id: 'reuse-secondhand-theme-2026',
    themes: ['リユース・中古品'],
    keywords: ['リユース', '中古品', '循環経済', '真贋', '在庫回転', '越境EC'],
    category: 'テーマ解説',
    icon: '♻️',
    date: '2026/08/09',
    title: 'リユース・中古品テーマを読み解く：成長の鍵は買取量ではなく、信頼と在庫回転',
    summary: 'リユース市場は循環経済と節約志向の追い風を受けますが、買取、真贋、在庫、販売チャネルの運営力が収益性を分けます。小売とは異なる見方を整理します。',
    body: `
H2: リユースは「中古品を売る小売」より、循環の仕組みを運営する事業
...
`,
  ...
  {
    id: 'yen-intervention-japan-us-policy-2026',
    themes: ['銀行・金融', '内需・ディフェンシブ', '円安メリット', '石油', 'LNG', '食品・飲料', '輸送・物流'],
    keywords: ['円買い介入', 'ドル円', '日銀', '財務省', '米財務省', '為替介入', '日米協調', 'インフレ', '原油', '金利差'],
    category: '市場分析',
    icon: '💴',
    date: '2026/08/09',
    title: '2026年のドル円介入をわかりやすく読む：日米協調、物価高、金利差は何を意味するのか',
    summary: '2026年に続いた円買い介入と日米協調行動を、仕組み、家計・企業への影響、日米の政策事情、原油・金利・財政との関係、日本株で確認すべき点まで整理します。',
    body: `
H2: はじめに：為替介入は「円を高くする魔法」ではない
...
H2: 結論：見るべきは「水準」よりも、政策と実体経済の接続である
...
> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。為替、金利、原油、政策、企業業績は変動します。投資判断は最新の公式統計・企業開示等を確認し、ご自身の責任で行ってください。`,
  ...
  {
    id: 'japan-current-policy-growth-strategy-2026',
    themes: ['AI半導体', '防衛・航空', '原子力発電', '国土強靭化計画', '銀行・金融', '造船'],
    keywords: ['日本政策', '高市内閣', '日本成長戦略', '責任ある積極財政', '17分野'],
    category: '市場分析',
    icon: '🏛️',
    date: '2026/07/22',
    title: '2026年の日本政策を投資家目線で読む：高市内閣の成長戦略・積極財政・17分野',
    summary: '日本成長戦略を軸に、AI・半導体、防衛、エネルギー、造船、国土強靱化、賃金、地方、金融政策と国内外の関連企業を整理します。',
    body: `
H2: 2026年7月時点の日本政策をどう読むか
...
- 内閣官房「日本成長戦略」（2026年7月21日閣議決定）
- 内閣官房「戦略17分野における主要な製品・技術等の官民投資ロードマップ」
- 経済産業省「AI・半導体産業基盤強化フレーム」
- 資源エネルギー庁「第7次エネルギー基本計画」
- 防衛省「国家安全保障戦略・国家防衛戦略・防衛力整備計画」`,
  },
  {
    id: 'nvidia-japan-physical-ai-collaboration-2026',
    themes: ['フィジカルAI・産業用ロボット', 'AI・クラウド', 'データセンター電力・冷却', '自動運転'],
    keywords: ['NVIDIA', 'エヌビディア', 'フィジカルAI', 'Cosmos', 'Noetra', 'Vera Rubin', '富士通', 'ファナック', '安川電機', '川崎重工業', 'Toyota', 'SoftBank'],
    category: '市場分析',
    icon: '🤖',
    date: '2026/07/22',
    title: 'NVIDIAと日本企業の大型協業を読み解く：フィジカルAI、Noetra、ロボット産業の勝機',
    summary: 'NVIDIAのジェンスン・フアンCEO来日に合わせて発表された、日本企業とのフィジカルAI、国家規模AI基盤、ロボット、Toyota、SoftBank、業界特化AIの協業を整理し、日本企業の収益機会とリスクを分析します。',
    body: `
H2: 2026年7月の発表は何だったのか
...
H3: 1. NVIDIA Cosmos Coalitionと日本の製造・ロボット企業
...
H3: 2. 富士通、ファナック、安川電機、川崎重工業の協調制御基盤
...
- NVIDIA Cosmosによる世界モデル
- NVIDIA Isaacによるロボット開発
- NVIDIA Omniverseによるデジタルツイン
- Newton物理エンジンによる動作シミュレーション
- 実機投入前の仮想検証
- 複数メーカーのロボットや設備を横断した協調制御
...
- GPUを販売するNVIDIAの売上
- データセンターを建設・運営する企業の収益
- 電力・冷却・ネットワーク設備の需要
- AIモデルを開発する企業の研究開発費
- 開発したモデルを実際の有料サービスへ変える企業
- 国費や補助金に依存する部分

巨大な設備が作られることと、日本企業の利益が直ちに増えることは同じではありません。費用負担、利用料金、設備稼働率、知的財産の帰属、モデルの商用化が重要です。

H3: 4. Toyota、Woven by Toyotaとの協業拡大

NVIDIAはToyotaとの協業を、自動車、工場、都市へ広げる方針を示しました。

主な領域は次のとおりです。

- NVIDIA DRIVE AGXとDriveOSを使った次世代運転支援
- 自動車向けコード生成・レビューAI
- OmniverseとIsaac Simを用いた工場シミュレーション
- 都市交通を理解するマルチモーダル視覚言語モデル
- Woven Cityを含む都市・インフラでのAI活用
...
- 富士通、ファナック、安川電機、川崎重工業による協調制御基盤
- NoetraによるVera Rubin世代の国家規模AIファクトリー
- Toyota、SoftBank、日立、NECなどによる産業・都市・通信への実装
...
- NVIDIA Newsroom「Japan’s Robotics and Manufacturing Leaders Build on NVIDIA Cosmos to Advance Physical AI Frontier」（2026年7月15日）
- NVIDIA Newsroom「Japan Government, Industrial Leaders and NVIDIA Launch the World’s First National AI Infrastructure」（2026年7月16日）
- NVIDIA Newsroom「Japan’s Enterprises and Startups Build Industry-Specialized AI With NVIDIA Nemotron Open Models」（2026年7月15日）
- NVIDIA Blog「NVIDIA and Japan Bring Full-Stack AI and Robotics to Every Industry」（2026年7月15日以降更新）
- 各参加企業の今後の適時開示、決算説明資料`,
  },
  {
    id: 'next-generation-semiconductors-history-present-future',
    themes: ['次世代半導体'],
    keywords: ['次世代半導体', '過去', '現在', '未来', '関連銘柄'],
    category: '産業・技術',
    icon: '🧪',
    date: '2026/07/22',
    title: '次世代半導体の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '次世代半導体の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 次世代半導体とは何か

次世代半導体は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

半導体は微細化だけで性能を高める時代から、GAA、EUV、3次元積層、チップレット、SiC・GaNなど複数技術を組み合わせる時代へ移っています。
...
AI向け先端ロジックとHBM、電気自動車向けパワー半導体、国内生産基盤の再構築が同時に進み、日本企業は材料、装置、検査、切断・研削で重要な位置を占めます。
...
将来は演算、メモリ、通信、電力効率、冷却を一体で最適化する競争になります。技術世代の移行、輸出規制、巨額設備投資と減価償却が変動要因です。
...
- キオクシアHD（285A）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- アドバンテスト（6857）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東京エレクトロン（8035）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日立製作所（6501）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 信越化学工業（4063）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ルネサス（6723）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ディスコ（6146）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- NEC（6701）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 富士通（6702）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- SUMCO（3436）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- レーザーテック（6920）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ソシオネクスト（6526）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
- 関連事業の売上、利益、受注、受注残
- 設備投資、減価償却、研究開発費
- 粗利益率、営業利益率、営業キャッシュフロー
- 顧客、地域、製品への依存度
- 原材料、為替、価格転嫁
- 有利子負債、増資、転換社債、株式報酬
- 市場規模ではなく、自社が獲得できる売上と利益
...
- 1週間から1か月：出来高、売買代金、材料発表後の持続性
- 3か月から1年：受注、売上、利益率、会社計画の上方・下方修正
- 3年以上：市場構造、競争優位、設備投資回収、技術方式の定着
...
次世代半導体は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'fusion-power-history-present-future',
    themes: ['核融合発電'],
    keywords: ['核融合発電', '過去', '現在', '未来', '関連銘柄'],
    category: 'エネルギー',
    icon: '☀️',
    date: '2026/07/22',
    title: '核融合発電の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '核融合発電の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 核融合発電とは何か

核融合発電は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

核融合研究は、軽い原子核が融合するときのエネルギーを発電へ使う試みです。磁場閉じ込め、レーザー方式などが長期に研究されてきました。
...
現在は大型国際実験と民間企業の高速開発が並行し、超伝導磁石、真空、極低温、電源、遠隔保守、耐熱材料など広い産業が関わります。
...
商用化時期は不確実ですが、周辺技術の需要は先行します。単一方式の勝敗より、複数方式で共通利用される機器・材料を確認する方が現実的です。
...
- 三菱重工業（7011）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日立製作所（6501）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- IHI（7013）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- NEC（6701）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 三菱電機（6503）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 富士通（6702）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 住友重機械工業（6302）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日本電子（6951）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
核融合発電は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'nuclear-power-history-present-future',
    themes: ['原子力発電'],
    keywords: ['原子力発電', '過去', '現在', '未来', '関連銘柄'],
    category: 'エネルギー',
    icon: '⚛️',
    date: '2026/07/22',
    title: '原子力発電の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '原子力発電の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 原子力発電とは何か

原子力発電は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

日本では高度成長期以降、輸入燃料依存を下げ安定電源を確保する目的で原子力発電が導入され、重工、電機、保守の産業基盤が形成されました。
...
事故後の規制強化を経て、再稼働、廃炉、安全対策、燃料、電力需要、脱炭素、エネルギー安全保障が同時に議論されています。
...
既存炉の長期運転、廃炉、次世代革新炉、SMRなどが焦点です。政策、地元同意、司法判断、工期、費用超過が大きなリスクです。
...
- 日立製作所（6501）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- IHI（7013）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東京電力HD（9501）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 関西電力（9503）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 中部電力（9502）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- GSユアサ（6674）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 住友重機械工業（6302）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 北海道電力（9509）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 九州電力（9508）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東北電力（9506）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
原子力発電は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'quantum-computing-history-present-future',
    themes: ['量子コンピューター'],
    keywords: ['量子コンピューター', '過去', '現在', '未来', '関連銘柄'],
    category: '産業・技術',
    icon: '⚛️',
    date: '2026/07/22',
    title: '量子コンピューターの過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '量子コンピューターの歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 量子コンピューターとは何か
...
- 三菱電機（6503）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 富士通（6702）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- NTT（9432）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 住友商事（8053）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- デンソー（6902）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  ...
  {
    id: 'wearable-devices-history-present-future',
    themes: ['ウェアラブル端末'],
    keywords: ['ウェアラブル端末', '過去', '現在', '未来', '関連銘柄'],
    category: '消費・技術',
    icon: '⌚',
    date: '2026/07/22',
    title: 'ウェアラブル端末の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: 'ウェアラブル端末の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: ウェアラブル端末とは何か

ウェアラブル端末は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

電子腕時計、補聴器、スポーツ計測器から発展し、スマートフォン普及後に活動量計とスマートウォッチが一般化しました。
...
通知、決済、運動、睡眠、心拍、位置情報が主用途で、センサー、通信、電池、表示、光学、防水など日本企業の部品技術が使われます。
...
健康モニタリング、遠隔医療、ARグラス、見守り、AI支援へ広がる一方、電池寿命、精度、個人情報、医療責任が課題です。
...
- ソニーグループ（6758）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 村田製作所（6981）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- TDK（6762）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ローム（6963）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 京セラ（6971）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- カシオ計算機（6952）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ニコン（7731）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- アルプスアルパイン（6770）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- シチズン時計（7762）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- セイコーグループ（8050）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
ウェアラブル端末は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'coatings-industry-history-present-future',
    themes: ['塗料'],
    keywords: ['塗料', '過去', '現在', '未来', '関連銘柄'],
    category: '素材',
    icon: '🎨',
    date: '2026/07/22',
    title: '塗料の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '塗料の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 塗料とは何か

塗料は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

塗料は装飾だけでなく腐食、紫外線、水分、薬品、熱から製品を守る材料として、自動車、建築、船舶、工業化とともに高機能化しました。
...
建築、自動車、船舶、包装、電子材料が需要先で、原料、為替、生産量、価格転嫁、海外事業の収益管理が業績を左右します。
...
水性、低VOC、粉体、防汚、抗菌、遮熱、自己修復、電池・半導体向け機能性材料が成長領域です。
...
- 日本ペイントHD（4612）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 関西ペイント（4613）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 中国塗料（4617）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- DIC（4631）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東洋インキSCHD（4634）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日本特殊塗料（4619）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 神東塗料（4616）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
塗料は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'security-services-history-present-future',
    themes: ['警備'],
    keywords: ['警備', '過去', '現在', '未来', '関連銘柄'],
    category: 'サービス',
    icon: '🛡️',
    date: '2026/07/22',
    title: '警備の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '警備の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 警備とは何か

警備は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

人的警備から、侵入センサーと通信回線による機械警備へ発展し、防災、救急、ビル管理、見守りへ事業領域が拡大しました。
...
人手不足と人件費上昇を背景に、防犯カメラ、画像認識、入退室管理、ロボット、クラウド監視を組み合わせる動きが進みます。
...
映像AIとロボットが巡回を担い、人は判断と緊急対応へ集中する方向です。誤検知、顔認証規制、プライバシー、事故責任が課題です。
...
- NEC（6701）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 三菱電機（6503）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- セコム（9735）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 綜合警備保障（ALSOK）（2331）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- キヤノン電子（7739）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東洋テック（9686）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
警備は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'wires-copper-history-present-future',
    themes: ['電線・銅'],
    keywords: ['電線・銅', '過去', '現在', '未来', '関連銘柄'],
    category: '素材・インフラ',
    icon: '🔌',
    date: '2026/07/22',
    title: '電線・銅の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '電線・銅の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 電線・銅とは何か

電線・銅は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

銅は導電性と加工性に優れ、発電、送配電、通信、自動車、家電の普及とともに需要を伸ばしました。日本企業は電力ケーブル、ワイヤーハーネス、光ファイバーなどへ展開しています。
...
再エネ系統接続、送電網更新、EV、半導体工場、AIデータセンターが需要を押し上げています。銅価格は売上、在庫評価、価格転嫁、運転資金を変動させます。
...
洋上風力、海底ケーブル、国際連系線、データ通信で重要性が増します。鉱山開発の長期化、景気後退、代替材料、資源国政策がリスクです。
...
- フジクラ（5803）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 古河電気工業（5801）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 住友電気工業（5802）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- SWCC（5805）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- タツタ電線（5809）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 三菱マテリアル（5711）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 住友金属鉱山（5713）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- JX金属（5016）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
電線・銅は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'advanced-packaging-substrates-history-present-future',
    themes: ['先端パッケージング・基板'],
    keywords: ['先端パッケージング・基板', '過去', '現在', '未来', '関連銘柄'],
    category: '半導体',
    icon: '🧩',
    date: '2026/07/22',
    title: '先端パッケージング・基板の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: '先端パッケージング・基板の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: 先端パッケージング・基板とは何か

先端パッケージング・基板は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

半導体パッケージはチップ保護と外部接続の後工程から、複数チップを近接配置して性能を決める中核技術へ変化しました。
...
AI向けGPUとHBMではインターポーザー、再配線、微細バンプ、封止材、基板大型化、検査が重要で、日本企業は材料、基板、装置に強みを持ちます。
...
チップレット、3次元積層、光電融合が進む一方、大型基板の反り、熱、歩留まり、設備投資、顧客集中がリスクです。
...
- イビデン（4062）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- メイコー（6787）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日本シイエムケイ（6958）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 太陽ホールディングス（4626）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- TOWA（6315）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- レゾナックHD（4004）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日本マイクロニクス（6871）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ディスコ（6146）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 東京応化工業（4186）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
先端パッケージング・基板は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'data-center-power-cooling-history-present-future',
    themes: ['データセンター電力・冷却'],
    keywords: ['データセンター電力・冷却', '過去', '現在', '未来', '関連銘柄'],
    category: 'インフラ',
    icon: '❄️',
    date: '2026/07/22',
    title: 'データセンター電力・冷却の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: 'データセンター電力・冷却の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: データセンター電力・冷却とは何か

データセンター電力・冷却は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

データセンターは大型計算機の集約施設からクラウドとインターネットを支える社会インフラへ発展し、受変電、UPS、非常用電源、空調、監視が不可欠です。
...
AIサーバーの高密度化で受電容量、配電、変圧器、UPS、空冷・液冷が導入速度を制約します。水使用、系統接続、工期も重要です。
...
高電圧直流、蓄電池、液浸冷却、排熱利用、AI設備制御が進む可能性があります。過剰建設、電力規制、工期遅延がリスクです。
...
- ニデック（6594）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 富士電機（6504）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 明電舎（6508）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 高砂熱学工業（1969）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 新晃工業（6458）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 日東工業（6651）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ダイヘン（6622）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 三社電機製作所（6882）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- サンケン電気（6707）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
データセンター電力・冷却は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'ma-business-succession-history-present-future',
    themes: ['M&A・事業承継'],
    keywords: ['M&A・事業承継', '過去', '現在', '未来', '関連銘柄'],
    category: '企業経営',
    icon: '🤝',
    date: '2026/07/22',
    title: 'M&A・事業承継の過去・現在・未来：市場構造と関連銘柄を徹底解説',
    summary: 'M&A・事業承継の歴史、現在の需要構造、将来シナリオ、関連銘柄、決算で確認すべき指標を整理します。',
    body: `
H2: M&A・事業承継とは何か

M&A・事業承継は、社会や産業の長期変化を一つの視点で整理するためのテーマです。ただし、テーマ名が同じでも、材料、部品、装置、システム、施工、保守、サービスでは利益が出る時期とリスクが異なります。テーマ投資では、話題性ではなく、需要がどの企業の売上と利益へつながるかを確認する必要があります。

H2: 過去

中小企業経営者の高齢化と後継者不足を背景に、親族内承継や廃業だけでなく第三者M&Aが選択肢として普及しました。
...
仲介会社は案件発掘、評価、資料作成、交渉、契約を支援し成功報酬を得ます。採用、生産性、案件品質、利益相反、手数料透明性が重要です。
...
需要は続く一方、競争、自主ルール、評判、規制が業界構造を変えます。成約件数だけでなく単価、成約率、1人当たり生産性、PMI支援を確認すべきです。
...
- 日本M&Aセンター（2127）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- M&A総研HD（9552）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ストライク（6196）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- オンデック（7360）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- 名南M&A（7076）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
- ブティックス（9272）：テーマ関連の製品・サービス・設備を持つ企業。関連事業の売上寄与は最新決算で確認が必要です。
...
M&A・事業承継は長期的な変化を含むテーマですが、テーマの成長と株価上昇は同じではありません。過去の発展経路、現在の需要と制約、将来の方式・制度、企業ごとの収益構造を分けて確認してください。

> 免責事項：本コラムは一般的な情報提供を目的とし、特定銘柄の取得・売却・保有を推奨しません。記事公開後に制度、企業情報、市場環境が変化する場合があります。投資判断は最新の企業開示等を確認し、ご自身の責任で行ってください。`,
  },
  {
    id: 'global-selloff-kospi-2026-july',
    themes: ['AI半導体', 'メモリ', '半導体製造装置', '石油', 'LNG', '銀行・金融'],
    keywords: ['世界株安', 'KOSPI', '韓国株', 'AIバブル', '半導体株', 'SKハイニックス', 'サムスン電子', 'SOX指数', '原油高', '金利'],
    category: '分析手法',
    icon: '🌐',
    date: '2026/07/20',
    title: '世界株安はAI相場の終わりか：KOSPI急落から読み解く2026年7月の市場調整',
    summary: '2026年7月に世界を襲ったAI・半導体株安を、韓国KOSPIの急落、指数集中、レバレッジ、原油高、金利上昇から分析します。日本株の半導体・メモリ・装置株への影響と、下げ止まりを確認する指標も整理します。',
    body: `
H2: 2026年7月の世界株安をどう見るか
...
H2: メモリ市況は強いのに、なぜSamsungとSK hynixが売られたのか
...
> 免責事項：本記事は2026年7月20日時点の公開情報を基に、市場環境を整理したものです。特定の金融商品の売買を推奨するものではありません。市場状況や企業業績は変化するため、最新の決算、開示資料、株価、為替、金利を確認したうえで、ご自身の判断と責任で投資を行ってください。`,
  },
  {
    ...
    themes: ['テーマ一覧'],
    keywords: ['テーマ選定基準', '構成銘柄', 'テーマ株', '採用基準', '除外基準', '銘柄更新'],
    category: '入門',
    icon: '🧭',
    date: '2026/07/19',
    title: 'StockWaveJPのテーマ選定基準：テーマと構成銘柄をどう決めているか',
    summary: 'StockWaveJPが公式テーマを新設・統合し、構成銘柄を採用・除外する際の判断基準を公開します。テーマとの事業関連性、売上・利益への寄与、開示情報、流動性、重複掲載の扱い、定期見直しの方法を解説します。',
    body: `
H2: テーマ選定基準を公開する理由
...
> 免責事項：本基準はStockWaveJPの編集・分類方針を示すものであり、特定銘柄の売買を推奨するものではありません。テーマ構成や算出方法は、サービス改善やデータ提供条件の変更に伴い見直す場合があります。`,
  },
  {
    ...
    themes: ['テーマ一覧'],
    keywords: ['大株主', '有価証券報告書', 'EDINET', '株主構成', '創業家', '機関投資家', '信託口'],
    category: '入門',
    icon: '🏛️',
    date: '2026/07/04',
    title: '大株主を読めば企業がわかる：有価証券報告書「大株主の状況」の実践的な読み方',
    summary: '同じ業績でも株価の動き方が違うのはなぜか。答えは株主構成にあります。創業家・信託口・外国機関など株主の5類型の見分け方と、前期比較から資金の流れを読む方法を解説します。',
    body: `
H2: なぜ「誰が株を持っているか」を見るのか
...
StockWaveJPのテーマ一覧で全72テーマの騰落率・出来高・売買代金を一覧でき、テーマ間の相対的な強弱を把握できます。騰落率ランキングで上位にいながら出来高も増加しているテーマは、資金流入が本格化している可能性が高く、最も注目すべき状態です。
...
StockWaveJPのテーマヒートマップは全72テーマの騰落率を色（赤＝上昇・緑＝下落）で一覧表示します。テーマヒートマップを一瞥するだけで「今の相場はどのテーマが強く、どのテーマが弱いか」という全体感を数秒で把握できます。「すべてのテーマが赤（全面高）」の状態は市場全体のリスクオン、「すべてのテーマが緑（全面安）」はリスクオフの局面を示します。「一部のテーマだけが強い」状態は資金ローテーションが起きているシグナルです。
...
StockWaveJPでは防衛・宇宙テーマの騰落率・出来高・売買代金をプラン別の更新頻度で確認できます。出来高の急増を確認してから詳細を調査するという使い方が効果的です。
...
StockWaveJPのモメンタムページでは、全72テーマの状態を一覧で確認できます。「🔥加速」が多い時は市場全体が強気、「❄️失速」が多い時は弱気のサインとして、相場全体の地合い判断にも活用できます。
...
StockWaveJPでは造船テーマの騰落率推移をプラン別の更新頻度で確認できます。他のテーマ（防衛・EV・脱炭素）との比較分析にもご活用ください。
...
`,
  },
  {
    ...
    icon: '🎩',
    date: '2026/03/27',
    title: 'バークシャーの日本商社株投資：確認できる事実と投資上の論点',
    summary: 'バークシャーの5大商社投資について、2026年時点の経営体制と一次資料に基づき整理します。',
    body: `H2: バークシャーの日本株投資
...
`,
  },
  {
    ...
    themes: ['光ファイバー・光部品', 'IOWN', '通信'],
    keywords: ['光通信', '光ファイバー', 'データセンター', 'AI', 'フォトニクス'],
    category: '光ファイバー・光部品',
    icon: '💡',
    date: '2026/03/23',
    title: '光ファイバー・光部品テーマ：AI・データセンター需要急増が生む光ファイバー・光部品市場の構造的拡大',
    summary: '生成AIの普及でデータ通信量が爆発的に増加しています。光ファイバー・光トランシーバー・光半導体など光通信インフラの需要急増と日本企業の強みを解説します。',
    body: `
H2: 光ファイバー・光部品テーマとは

光ファイバー・光部品テーマは、光ファイバーケーブル・光トランシーバー・光増幅器・光スイッチ・フォトニクスデバイスなど、光を利用した通信インフラ・部品を製造・販売する企業を対象とした投資テーマです。ChatGPT登場以降の生成AIブームによりデータセンターでの大規模な演算処理需要が爆発的に増加し、その通信バックボーンとなる光通信インフラへの需要も急拡大しています。2024〜2026年は光ファイバー・光部品テーマにとって「歴史的な需要ブーム」の時期に当たります。

H2: なぜ今、光ファイバー・光部品テーマが注目されるのか
...
光ファイバー・光部品テーマはデータセンター向けだけでなく、海底ケーブル（海を横断する光ファイバー通信網）の分野でも成長が続いています。グーグル・アマゾン・メタなどのハイパースケーラーが自社専用の海底ケーブル敷設を積極的に進めており、通信会社向けだけだった海底ケーブル市場が大きく変化しています。
...
古河電気工業（5801）は光ファイバー・光部品に加え、自動車向けワイヤーハーネス・熱管理材料でも高い競争力を持ちます。光ファイバー・光部品テーマとEV・自動車テーマ双方の恩恵を受けられる複合素材メーカーです。

H2: 光ファイバー・光部品テーマの課題とリスク
...
光ファイバー・光部品テーマをデータで観察していると、AI・データセンター関連のニュース（エヌビディア決算・ハイパースケーラーの設備投資計画）に対して特に敏感に反応することが確認されます。エヌビディアが「次世代GPUの出荷加速」を発表した際に光ファイバー・光部品テーマの出来高が急増するパターンは、光部品がAIインフラの「必須部品」として位置づけられていることを示しています。

当編集部が注目しているのは「半導体テーマの動向と光ファイバー・光部品テーマの連動性」です。半導体テーマが加速モメンタムにある時期は、光ファイバー・光部品テーマも同時に強くなる傾向があります。逆に半導体が調整局面に入ると光通信も同様に調整することが多いです。この連動性を理解した上で、「半導体テーマが強い局面で光ファイバー・光部品テーマにも注目する」という戦略が有効と考えています。

また、NTTのIOWN構想は長期的に日本の光通信産業全体を押し上げる可能性のある大型国家プロジェクトです。実用化・商用化のスケジュールや技術実証の進捗が具体化するたびに光通信関連株がポジティブに反応するパターンを今後も注視しています。

H2: まとめと今後の展望

光ファイバー・光部品テーマはAI時代のデータセンター建設ラッシュという「今この瞬間の強い追い風」と、IOWNに代表される「次世代光電融合技術」という中長期の成長ストーリーを併せ持つ投資テーマです。住友電工・浜松ホトニクス・古河電工・NECなど、グローバルな競争力を持つ日本企業が多い点も魅力です。
...
`,
  },
  {
    ...
    icon: '🌍',
    date: '2026/03/28',
    title: '中東・イラク情勢と日本株：確認すべきエネルギー経路とリスク',
    summary: '中東情勢を断定的な確率予測ではなく、ホルムズ海峡・エネルギー価格・企業コストへの波及経路から整理します。',
    body: `H2: イラク情勢を見る際の前提
...
`,
  },
  {
    ...
    date: '2026/04/11',
    themes: ['光ファイバー・光部品', 'IOWN', 'AIデータセンター'],
...
このような一年で株価が3倍になる銘柄は、後から振り返ると「なぜあの時買えなかったのか」と悔やまれますが、StockWaveJPで光ファイバー・光部品テーマの出来高急増・モメンタム「加速」を確認していれば、早期にエントリーできた可能性があります。
...
フジクラは光ファイバー・光部品テーマと再生可能エネルギーテーマの双方に連動する「クロステーマ銘柄」として継続観察しています。この二つのテーマが同時に「加速」モメンタムとなっているときはフジクラへの複合的な追い風となり、最も株価が上昇しやすい局面です。
...
`,
...

  {
    ...
    date: '2026/04/11',
    themes: ['光ファイバー・光部品', 'EV・電気自動車', 'IOWN'],
    keywords: ['古河電工','5801','光ファイバー','電力ケーブル','ワイヤーハーネス','EV','データセンター','銅'],
    summary: '古河電気工業（5801）は光ファイバー・電力ケーブル・自動車用ワイヤーハーネス・銅製品を手掛ける総合電線メーカー。AIデータセンター向け光ファイバー需要急増と洋上風力向け電力ケーブルという二大成長テーマを背景に株価が上昇。競合のフジクラ・住友電工とともに光ファイバー・光部品テーマの中核銘柄を徹底解説します。',
    body: `
H2: 古河電気工業とはどんな会社か
...
古河電工は光ファイバー・光部品テーマと再生可能エネルギーテーマの双方に連動するクロステーマ銘柄として観察しています。フジクラと同様に「AI・データセンター需要の強さ＋再エネ政策の追い風」が重なる局面でこの銘柄への資金流入が加速するパターンが見られます。

電線御三家（古河・フジクラ・住友電工）の中で古河電工は相対的にバリュエーションが低い局面が多く、「フジクラが先行して上昇した後に古河電工にローテーションする」というパターンも観察されます。StockWaveJPの光ファイバー・光部品テーマの出来高ランキングでフジクラ・住友電工に加え古河電工が上位に現れ始めたとき、テーマ全体のモメンタムが広がっているサインとして注目します。
...
`,
  },
  {
    ...
    date: '2026/05/29',
    summary: '京都発祥の電子部品メーカー・村田製作所が2026年5月、時価総額14兆円を突破して任天堂を抜いて京都No.1企業となりました。世界最強の「材料技術」を武器に、AI・EV時代をどう勝ち抜くのか。業績・戦略・リスクを徹底分析します。',
    body: `
## 村田製作所とはどんな会社か
...
`,
  },
  {
    ...
    theme: 'MLCC・電子部品',
    themes: ['MLCC・電子部品'],
    title: 'MLCC（積層セラミックコンデンサ）産業分析：AI・EV時代の最重要部品と日本電子部品メーカーへの影響',
    date: '2026/05/29',
    summary: 'AIサーバー1台に使われるMLCC（積層セラミックコンデンサ）はスマートフォンの10〜20倍。世界シェアNo.1の村田製作所を軸に、MLCC産業の構造変化と日本の電子部品メーカーへの影響を徹底解説します。',
    body: `
## MLCCとは何か――電子機器の「縁の下の力持ち」
...
`,
  },
  {
    id: 'spacex-analysis',
    themes: ['宇宙・衛星', '防衛・航空'],
    keywords: ['SpaceX', 'SPCX', 'イーロン・マスク', 'Starlink', 'IPO', 'ロケット', '個別銘柄'],
    category: '個別銘柄',
    icon: '🚀',
    title: 'SpaceX（SPCX）IPO分析：一次資料で確認できる事実とリスク',
    date: '2026/06/12',
    summary: 'SpaceXの2026年IPOについて、SEC・会社発表で確認できる条件と投資上の論点を整理します。',
    body: `SpaceXは2002年に設立された宇宙輸送・衛星通信企業です。2026年6月12日にNasdaqおよびNasdaq TexasへSPCXとして上場しました。
